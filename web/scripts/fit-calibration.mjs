...
async function main() {
  ...
  console.log(
    ...
    data[0].raw.toFixed(1),
    "to",
    data[data.length - 1].raw.toFixed(1),
  );
  console.log(
    "Actual age range:",
    Math.min(...data.map((d) => d.actual)),
    "to",
    Math.max(...data.map((d) => d.actual)),
  );
  ...
  printModelEvaluation(
    "2. Piecewise Linear (2 segments, auto threshold)",
    piecewiseModel,
    piecewiseEval,
  );
