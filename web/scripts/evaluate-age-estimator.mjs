...
async function main() {
  ...
  await Promise.all([
    ensureExists(metadata, 'metadata'),
    ensureExists(model, 'ONNX model'),
  ]);
...
for (const bucket of perDecade) {
  ...
  console.log(
    `${formatDecade(bucket.decade)} -> n=${stats.count
...
console.log(` slope=${calibration.slope.toFixed(4)}, intercept=${calibration.intercept.toFixed(2)}`);
console.log(`Calibrated MAE: ${calibratedStats.mae.toFixed(2)} | RMSE: ${calibratedStats.rmse.toFixed(2)} | Bias: ${calibratedStats.bias.toFixed(2)}`);
console.log(`Calibrated gender accuracy (unchanged): ${(calibratedStats.genderAccuracy * 100).toFixed(1)}%`);
...
console.log(' Anchors:', anchors.map((a) => `[${a.predicted.toFixed(1)}→${a.actual.toFixed(1)}]`).join(' '));
